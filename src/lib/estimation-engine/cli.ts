...
program
  ...
  .action(async (options) => {
    try {
      const estimate = await estimationEngine.calculateProjectEstimate(options.projectId)
      ...
      if (options.format === 'json') {
        ...
        console.log(`Total Estimate: ${estimate.totalMinHours}h - ${estimate.totalMaxHours}h`)
        ...
        estimate.taskBreakdown.forEach(task => {
          ...
          console.log(`    Subtotal: ${task.subtotalMinHours}h - ${task.subtotalMaxHours}h`)
          console.log('')
        })
      }
    } catch (error) {
      console.error('Error calculating estimate:', error instanceof Error ? error.message : error)
      process.exit(1)
    }
  })
...
program
  ...
  .action(async (options) => {
    try {
      ...
      console.error('Error recalculating:', error instanceof Error ? error.message : error)
      process.exit(1)
    }
  })
